...
const getLastPaymentStatus = (order) => {
    ...
    for (const paymentCollection of order.payment_collections) {
        if (utils_1.MathBN.gt(paymentCollection.captured_amount ?? 0, 0) ||
            ...
            paymentStatus[PaymentStatus.CAPTURED] += utils_1.MathBN.gte(paymentCollection.captured_amount, paymentCollection.amount)
                ? 1
                : 0.5;
        }
        if (utils_1.MathBN.gt(paymentCollection.refunded_amount ?? 0, 0)) {
            paymentStatus[PaymentStatus.REFUNDED] += utils_1.MathBN.gte(paymentCollection.refunded_amount, paymentCollection.amount)
                ? 1
                : 0.5;
        }
