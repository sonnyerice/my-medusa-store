...
function validateApplicationMethodAttributes(data, promotion) {
    ...
    if (promotion?.type === utils_1.PromotionType.BUYGET) {
        ...
        if (!(0, utils_1.isPresent)(maxQuantity)) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `application_method.max_quantity is a required field for Promotion type of ${utils_1.PromotionType.BUYGET}`);
        }
        if (!(0, utils_1.isPresent)(applyToQuantity)) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `application_method.apply_to_quantity is a required field for Promotion type of ${utils_1.PromotionType.BUYGET}`);
        }
        if (utils_1.MathBN.lt(maxQuantity, applyToQuantity)) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `max_quantity (${maxQuantity}) must be greater than or equal to apply_to_quantity (${applyToQuantity}) for BUYGET promotions.`);
        }
    }
