...
class TaxModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateForModels) {
    ...
    async createTaxRates(data, sharedContext = {}) {
        ...
        const serialized = await this.baseRepository_.serialize(rates);
    ...
    async getTaxLines(items, calculationContext, sharedContext = {}) {
        ...
        const toReturn = items.map((item) => {
            ...
            const candidateRates = allCandidateRates.filter((rate) => this.rateMatchesQuery(rate, rateQuery));
        ...
        for (const ruleCondition of $or ?? []) {
            ...
            if (ruleCondition.rules) {
                ...
                for (const ruleQuery of ruleQueries) {
                    if ([...(rate.rules ?? [])]?.some((rule) => rule.reference === ruleQuery.reference &&
...
__decorate([
    ...
    __metadata("design:paramtypes", [Array, Object]),
    __metadata("design:returntype", Promise)
], TaxModuleService.prototype, "createTaxRegions_", null);
__decorate([
    (0, utils_1.InjectManager)(),
    (0, utils_1.EmitEvents)()
    // @ts-expect-error
