...
function createAndConfigureHandler(context, stepName, config, input, invokeFn, compensateFn) {
    const handler = create_step_handler_1.createStepHandler.bind(context)({
        stepName,
        input,
        invokeFn,
        compensateFn,
...
function applyStep({ stepName, stepConfig = {}, input, invokeFn, compensateFn, }) {
    return function () {
        ...
        const handler = createAndConfigureHandler(this, stepName, stepConfig, input, invokeFn, compensateFn);
        ...
        refRet.config = (localConfig) => {
            ...
            const handler = createAndConfigureHandler(this, newStepName, newConfig, input, invokeFn, compensateFn);
...
function wrapConditionalStep(input, condition, handle) {
    const originalInvoke = handle.invoke;
    handle.invoke = async (stepArguments) => {
        let args = (0, helpers_1.resolveValue)(input, stepArguments);
